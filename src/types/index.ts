...
export type EquipmentType  = 'Dry Van' | 'Reefer' | 'Flatbed' | 'Step Deck' | 'RGN';
export type Theme          = 'dark' | 'light';
export type PageId         =
  | 'dashboard' | 'dispatch' | 'loads'
...
export interface Driver {
  ...
  st:     string;
  status: EntityStatus;
  cid:    number;   // carrier id (0 = unassigned)
  miles:  number;
}
...
export interface Load {
  id:     number;
  num:    string;
  bid:    number;   // broker id
  cid:    number;   // carrier id
  oCity:  string;
  oSt:    string;
  dCity:  string;
  dSt:    string;
...
// ─── App-wide state shape (passed as props or via context) ────────────────────
