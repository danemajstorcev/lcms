...
const HEADERS = ['Driver', 'Contact', 'CDL / State', 'Carrier', 'Miles', 'Status', ''];

export default function DriversPage({ drivers, setDrivers, carriers }: DriversPageProps) {
  ...
  function handleSave(data: Omit<Driver, 'id'>) {
    if (modal?.mode === 'add') {
      ...
      setDrivers((prev) => prev.map((d) => (d.id === modal.driver.id ? { ...data, id: modal.driver.id } : d)));
    }
    setModal(null);
  }
  ...
  return (
    <div className="fade-in">
      <PageHeader title="Drivers">
        <SearchInput value={searchQuery} onChange={setSearch} placeholder="Search drivers…" />
      ...
      <TableShell headers={HEADERS}>
        {slice.length === 0 && (
          <tr><td colSpan={HEADERS.length} className="table__empty">NO DRIVERS FOUND</td></tr>
        )}
        {slice.map((driver) => {
          const carrier = findById(carriers, driver.cid);
          return (
            <tr key={driver.id}>
              <td style={{ padding: '12px 14px', fontWeight: 600, fontSize: 14 }}>{driver.name}</td>
              ...
              <td style={{ padding: '12px 14px', fontSize: 13 }}>
                {carrier ? carrier.name : <span className="tc-muted">Unassigned</span>}
              </td>
              <td style={{ padding: '12px 14px' }} className="ff-mono fz-13">
                {driver.miles.toLocaleString()}
              </td>
              <td style={{ padding: '12px 14px' }}><Badge value={driver.status} /></td>
              <td style={{ padding: '6px 8px' }}>
                <div style={{ display: 'flex', gap: 2 }}>
                  <IconButton onClick={() => setModal({ mode: 'edit', driver })} title="Edit">✎</IconButton>
                  <IconButton onClick={() => handleDelete(driver.id)} title="Delete" danger>✕</IconButton>
                </div>
              </td>
            </tr>
          );
