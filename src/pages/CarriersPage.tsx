...
const HEADERS = ['Company', 'Contact', 'MC / DOT', 'Equipment', 'Loads', 'Status', ''];

export default function CarriersPage({ carriers, setCarriers, loads }: CarriersPageProps) {
  ...
  const filtered = useMemo(() => {
    return carriers.filter((c) => {
      ...
      const matchesSearch = !searchQuery || c.name.toLowerCase().includes(q) || c.mc.toLowerCase().includes(q);
  ...
  function handleSave(data: Omit<Carrier, 'id'>) {
    if (modal?.mode === 'add') {
      ...
      setCarriers((prev) => prev.map((c) => (c.id === modal.carrier.id ? { ...data, id: modal.carrier.id } : c)));
    }
    setModal(null);
  }
  ...
  return (
    <div className="fade-in">
      <PageHeader title="Carriers">
        <SearchInput value={searchQuery} onChange={setSearch} placeholder="Search carriers…" />
        <select value={statusFilter} onChange={(e) => setStatus(e.target.value)} style={{ maxWidth: 130 }}>
          <option>All</option>
          <option>Active</option>
          <option>Inactive</option>
        </select>
        <select value={equipFilter} onChange={(e) => setEquip(e.target.value)} style={{ maxWidth: 145 }}>
      ...
      <TableShell headers={HEADERS}>
        {slice.length === 0 && (
          <tr><td colSpan={HEADERS.length} className="table__empty">NO CARRIERS FOUND</td></tr>
        )}
        {slice.map((carrier) => {
          const carrierLoads = loads.filter((l) => l.cid === carrier.id).length;
          return (
            <tr key={carrier.id}>
              <td style={{ padding: '12px 14px', fontWeight: 600, fontSize: 14 }}>{carrier.name}</td>
              ...
              <td style={{ padding: '12px 14px' }}><EquipBadge value={carrier.equip} /></td>
              <td style={{ padding: '12px 14px' }} className="ff-mono fz-13">{carrierLoads}</td>
              <td style={{ padding: '12px 14px' }}><Badge value={carrier.status} /></td>
              <td style={{ padding: '6px 8px' }}>
                <div style={{ display: 'flex', gap: 2 }}>
                  <IconButton onClick={() => setModal({ mode: 'edit', carrier })} title="Edit">✎</IconButton>
                  <IconButton onClick={() => handleDelete(carrier.id)} title="Delete" danger>✕</IconButton>
                </div>
              </td>
            </tr>
          );
