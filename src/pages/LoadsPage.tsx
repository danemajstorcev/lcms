...
type ModalState = { mode: "add" } | { mode: "edit"; load: Load } | null;
...
export default function LoadsPage({
  ...
  function handleSave(data: Omit<Load, "id">) {
    if (modal?.mode === "add") {
      ...
      setLoads((prev) =>
        prev.map((l) =>
          l.id === modal.load.id ? { ...data, id: modal.load.id } : l,
        ),
      );
    }
    setModal(null);
          ...
          return (
            <tr key={load.id}>
              <td
                ...
                className="ff-mono fz-11 tc-accent fw-600"
              >
                {load.num}
              </td>
              ...
              <td
                style={{ padding: "11px 14px" }}
                className="ff-mono fz-11 tc-sub"
              >
                {load.pDate}
                <br />
                {load.dDate}
              ...
              <td
                style={{ padding: "11px 14px" }}
                className="ff-mono fz-13 tc-sub"
              >
                {formatCurrency(load.pay)}
              </td>
              <td
                style={{ padding: "11px 14px" }}
              ...
              <td style={{ padding: "6px 8px" }}>
                <div style={{ display: "flex", gap: 2 }}>
                  <IconButton
                    onClick={() => setModal({ mode: "edit", load })}
                    title="Edit"
                  >
                    ✎
                  </IconButton>
