...
export default function DispatchBoard({ loads, setLoads, carriers, brokers }: DispatchBoardProps) {
  ...
  function handleDrop(status: LoadStatus) {
    ...
    setLoads((prev) => prev.map((l) => (l.id === draggingId ? { ...l, status } : l)));
    setDraggingId(null);
    setOverStatus(null);
  }
  ...
  function handleSave(data: Omit<Load, 'id'>) {
    if (!editing) return;
    setLoads((prev) => prev.map((l) => (l.id === editing.load.id ? { ...l, ...data } : l)));
    setEditing(null);
  }
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div
              ...
              style={{ borderTop: `3px solid ${col.color}`, borderColor: isOver ? col.color : undefined }}
            ...
            >
              <div className="kanban-col__header">
                ...
                <span
                  ...
                  style={{ background: `${col.color}22`, color: col.color, borderColor: `${col.color}44` }}
                >
                  {colLoads.length}
                </span>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="kanban-card__top">
                      ...
                      <IconButton onClick={() => setEditing({ load })} title="Edit load">
                        ✎
                      </IconButton>
                    </div>
                    ...
                    <div className="kanban-card__meta">
                      {carrier?.name ?? 'Unassigned'} · {load.mi.toLocaleString()} mi
                    </div>

                    <div className="kanban-card__bottom">
                      <span className="kanban-card__rate">{formatCurrency(load.rate)}</span>
                      <span className="kanban-card__profit" style={{ color: profit >= 0 ? 'var(--clr-success)' : 'var(--clr-danger)' }}>
